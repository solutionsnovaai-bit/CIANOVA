...
function heroMouseParallax() {
  ...
  hero.addEventListener('mousemove', e => {
    ...
    gsap.to(logo, { x: x *  16, y: y *  11, duration: 1.5, ease: 'power2.out', overwrite: 'auto' });
  });
}
...
function initReveal() {
  document.querySelectorAll('[data-reveal]').forEach(el => {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => gsap.to(el, { opacity: 1, y: 0, duration: .8, delay, ease: 'power3.out' })
    });
  });
}
...
function initBonusReveals() {
  document.querySelectorAll('.bonus-item').forEach((item, i) => {
    ScrollTrigger.create({
      ...
      onEnter: () => gsap.to(item, { opacity: 1, y: 0, duration: .6, delay: i * .07, ease: 'power3.out' })
    });
  });
}
...
function initCompare() {
  ...
  if (!slider) return;
  const after  = slider.querySelector('.compare__pane--after');
  const handle = document.getElementById('compare-handle');
  let active = false;

  function update(x) {
    const r = slider.getBoundingClientRect();
    const pos = Math.max(5, Math.min(95, (x - r.left) / r.width * 100));
    after.style.clipPath      = `inset(0 ${100 - pos}% 0 0)`;
    handle.style.left         = pos + '%';
  }
  slider.addEventListener('mousedown',  e => { active = true; update(e.clientX); });
...
async function initScrub() {
  ...
  // Try frames/cia_001..048.webp — fall back to single image
  ...
  // Attempt real frames silently (replace slots if they load)
  for (let i = 1; i <= FRAME_COUNT; i++) {
    const img  = new Image();
    const idx  = i - 1;
    const n    = String(i).padStart(3, '0');
    img.onload = () => { frames[idx] = img; };
    img.src    = `assets/frames/cia_${n}.webp`;
  }

  const ZOOM = 1.06;
  function draw(frameIndex) {
    const img = frames[Math.max(0, Math.min(FRAME_COUNT - 1, Math.floor(frameIndex)))];
...
function initDemo() {
  ...
  // Check if video source will load
...
function initKineticHeads() {
  ...
  const heads = ['.dor__h2', '.compare__h2', '.scrub-h2', '.demo__h2',
                 '.dominar__h2', '.prova__h2', '.bonus__h2',
